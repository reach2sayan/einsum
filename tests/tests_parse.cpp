...
// subscript einsum<"..."> accepts and einsum(string) refuses would be two
...
static_assert(*parse_subscripts("ij,jk->ik") ==
              expect({"ij", "jk"}, "ik", true));
static_assert(*parse_subscripts("ij,jk") == expect({"ij", "jk"}, "ik", false));
static_assert(*parse_subscripts(" i j , j k -> i k ") ==
              expect({"ij", "jk"}, "ik", true));
static_assert(*parse_subscripts("ba") == expect({"ba"}, "ab", false));
static_assert(*parse_subscripts("ab,cd") ==
              expect({"ab", "cd"}, "abcd", false));
...
static_assert(failed_with(parse_subscripts("ij->ik"),
                          errc::unknown_output_label));
static_assert(failed_with(parse_subscripts("ij->ii"),
                          errc::repeated_output_label));

} // namespace

// Every subscript the suite exercises, so the data-driven pass below covers
// exactly what the static_asserts above do.  At namespace scope, not in the
// anonymous one: BOOST_DATA_TEST_CASE names it from its own generated class.
constexpr std::string_view kCorpus[]{"ij,jk->ik",
                                     "ij,jk",
                                     " i j , j k -> i k ",
                                     "ba",
                                     "ab,cd",
...
// One reported case per subscript rather than one loop that stops at the first
// surprise: the corpus is data, so Boost.Test drives it as data.
BOOST_DATA_TEST_CASE(ParseTest_FrontEndsAgree,
                     boost::unit_test::data::make(kCorpus), source) {
  const auto compile_time = parse_subscripts(source);
  const auto run_time = es::parse_subscript(source);
...
BOOST_AUTO_TEST_CASE(ParseTest_RuntimeMatchesTheStaticCorpus) {
  BOOST_CHECK(*es::parse_subscript("ij,jk->ik") ==
              expect({"ij", "jk"}, "ik", true));
  BOOST_CHECK(*es::parse_subscript("ba") == expect({"ba"}, "ab", false));
  BOOST_CHECK(*es::parse_subscript("ab,cd") ==
              expect({"ab", "cd"}, "abcd", false));
  BOOST_CHECK(*es::parse_subscript("ii->") == expect({"ii"}, "", true));
}

BOOST_AUTO_TEST_CASE(ParseTest_PlanIsTheSameEitherWay) {
  const auto from_text =
      es::parse_subscript("ij,jk,kl->il").and_then(es::impl::make_plan);
...
BOOST_AUTO_TEST_CASE(ParseTest_ErrorsFormat) {
  const auto bad = es::einsum("ij,,jk");
